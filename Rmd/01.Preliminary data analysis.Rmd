...
Interesting negative correlation with pd1 but not significant.  
...
```{r}
immune_data %>%
  ggplot(aes(x= bm_plasma_cell_percent_at_pre_ld, y= cd138_positive_cells, color= mrn))+
  geom_point()+
  scale_color_viridis()+
  theme(legend.position = "none",
        text = element_text(size = 10)) +
  ...
  stat_regline_equation(size = 2, label.x = 50,
                        label.y.npc = 0.85)

immune_data %>%
  ggplot(aes(x= plasma_cell_leukemia_yes_no, y= cd138_positive_cells))+
  geom_boxplot()+
  geom_jitter()
```
...
table_var <- c("extramedullary_disease_yes_no", 
               ...
               "infection_yes_no", "need_for_gcsf",
               ...
               "CRS_any", "CRS_gradecat", "gr3_CRS", "gr2_CRS",
               "ICANS_any", "ICANS_any2", "ICANS_gradecat",
...
for (i in table_var){
  ...
  tbl <- immune_data %>% select(i, starts_with("percent")) %>% 
    tbl_summary(by = i) %>% 
    bold_labels() %>% 
    add_p() %>% bold_p(t= 0.05) %>% 
...
table_var <- c("day30response_v2", "mon3response_v2", "mon6response_v2", "mon9response_v2", 
               ...
               "ORR_15mon", "ORR_18mon", "best_ORR", "best_response", 
               ...
               "CRorbetter_18mon", "best_CRorbetter"
)
...
rate_data <- immune_data %>%
  select(mrn, total_cells : cd3plus_pd1plus_lag3plus_tim3plus_cells) %>%
  mutate(across(where(is.numeric), ~ . / cd138_positive_cells)) %>%
  `colnames<-`(c("mrn", str_c(colnames(.)[2:ncol(.)], "_plasma_cells_rate")))
...
```{r}
immune_data %>%
  select(ends_with("_plasma_cells_rate")) %>%
  tbl_summary() %>%
  bold_labels()
```
